...
function conductValidation($parameter)
{
    ...
    if ($parameter == 'edit_details')
    {

        return validateTextInputEdit() && validateForAlphaChars() 
        && validateEmail() && validateDropDownSelection();
    }
}
...
function conductRegistration($parameter){
    ...
    { 
       setErrorMsg(registerUser(getFormData()), PATH);
    }     
}
...
{
    $fields = array('f_name', 'l_name', 'email_add', 'security_quest', 'security_ans_input');
    $data = array();
    $counter = 0;
...
function validateTextInputEdit()
{
    $fields = array('f_name', 'l_name', 'email_add', 'security_ans_input');
